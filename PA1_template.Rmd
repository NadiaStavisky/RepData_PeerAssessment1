...
ttl_hlines <- data.frame(daytype = c("weekday", "weekend"), mean_val = c(wd_mean, we_mean), median_val = c(wd_median, we_median))
  ...
  daytype_plot + geom_line(show.legend = TRUE) +
    facet_grid(daytype ~.) +
  geom_hline(aes(yintercept = mean_val, colour = "mean"), data = ttl_hlines, color = "red", show.legend = TRUE) +
  geom_hline(aes(yintercept = median_val, colour = "median"), data = ttl_hlines, color = "green", show.legend = TRUE) +
